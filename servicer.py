...
class RAFTServiceServicer(raft_pb2_grpc.RAFTServiceServicer):
    ...
    def setup_background_tasks(self): # function to set up background tasks on init
        ...
        self.send_heartbeat_thread = threading.Thread(target=self.send_heartbeat) # start thread to send hearbeats to heartbeat server
    ...
    def output(self, text):
        if os.path.exists(self.output_file_path) and os.path.getsize(self.output_file_path) > 0: # if already exists add new line before saying server on
                text = "\n" + text
        ...
        while not self.shutdown_event.is_set(): # do constantly until shutdown
            ...
            for id, last_time in list(self.last_heartbeat.items()):
                ...
                if current_time - last_time > self.heartbeat_timer+3 and id == self.leaderId: # if has been 3s past expected heartbeat reception, it might be down
                    ...
                    if not self.voted: self.start_election_timer() # start an election if you havent voted for someone already
                    else: self.output("cant start a new election bc already voted!")
